# -*- coding: utf-8 -*-
from exparser.EXparser.Segment_F1 import *
from exparser.JsonParser import *
from exparser.configs import *
from exparser.logger import *
...
_recheck          = _configs['recheck_E'];

_chunk_size       = _configs['chunk_size_E'];       # how many batch insert in the context of the elasticsearch bulk api
_scroll_size      = _configs['scroll_size_E'];      # how many input docs to retrieve at a time from the index
_max_extract_time = _configs['max_extract_time_E']; # minutes
_max_scroll_tries = _configs['max_scroll_tries_E']; # how often to retry when queries failed
_request_timeout  = _configs['request_timeout'];

_input_indicator    = "has_cermine_layout"
_output_field       = 'exparser_references_from_cermine_layout'
...
_scr_query = {"query":{ "bool":{ "must":{"term":{_input_indicator:True}},"must_not":{"term":{_output_indicator:True}}}}} if not _recheck else {"query":{ "bool":{ "must":{"term":{_input_indicator:True}}}}};  # which documents to select for further processing
    ...
    while returned > 0:
        for doc in page['hits']['hits']:
            try:
                ...
                extracted_exparser_references, success      = extract_segment_exparser(cermine_layout) if cermine_layout and isinstance(cermine_layout, str) else ([], False)
                body                                        = copy(_body)
                body['_id']                                 = doc['_id']
                body['_source']['doc'][_output_field]       = extracted_exparser_references if success else []
                body['_source']['doc'][_output_indicator]   = success
                body['_source']['doc'][_output_min1]        = len(extracted_exparser_references)>0 if success else False
                ...
                body['_source']['doc'][_output_min1]        = False
...
i = 0
for success, info in bulk(_client, update_exparser_references(), chunk_size=_chunk_size,request_timeout=_request_timeout):
    i += 1
    print('######', i, '#######################################################################')
    if not success:
